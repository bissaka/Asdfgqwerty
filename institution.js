...
issueBtn.addEventListener("click", async () => {
  try {
    ...
    await issueOnChain(signer, cid, contentHash, originalFile.name, encryptionKey);
  } catch (error) {
...
async function issueOnChain(signer, cid, contentHash, originalFilename, encryptionKey) {
  ...
  updateStatusHTML(`Sending transaction with Wormhole fee (${ethers.utils.formatEther(
    fee
  )} ETH)...`);

  // Call the 'issueCredential' function on your contract with content hash
  const tx = await issuerContract.issueCredential(cid, contentHash, { value: fee });

  updateStatusHTML(`Transaction sent. Waiting for confirmation...<br>Tx Hash: <a href="https://sepolia.etherscan.io/tx/${tx.hash}" target="_blank">${tx.hash}</a>`);
  ...
  alert("Credential issued successfully! Your Secret Key File will now be downloaded.");
...
async function handleBatchIssuance() {
  ...
  updateStatus("Secret key file downloaded. Please confirm to proceed with on-chain transaction.");
