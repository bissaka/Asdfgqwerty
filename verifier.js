...
async function checkBlockchain(network, cid) {
  ...
  if (isRevoked) {
    throw new Error(
      "Verification Failed: This credential has been revoked."
    );
  }
...
async function processCredentialFile(cid, issuerAddress) {
  updateStatusHTML(`✅ <strong>On-Chain Verification Successful!</strong><br>Issued by: ${issuerAddress}<br>Fetching encrypted file from IPFS...`);
  ...
  try {
    ...
    updateStatusHTML(`✅ <strong>Credential Verified and Decrypted Successfully!</strong><br>Issued by: ${issuerAddress}`);
  } catch (e) {
